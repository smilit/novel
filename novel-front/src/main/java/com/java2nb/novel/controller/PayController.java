...
public class PayController extends BaseController {
	...
	*/
    ...
    @SneakyThrows
    @PostMapping("wxPay")
    public void wxPay(Integer payAmount, HttpServletRequest request, HttpServletResponse httpResponse) {
        UserDetails userDetails = getUserDetails(request);
        if (userDetails == null) {
            //未登录，跳转到登陆页面
            httpResponse.sendRedirect("/user/login.html?originUrl=/pay/wxPay?payAmount="+payAmount);
        } else {
            //创建充值订单
            Long outTradeNo = orderService.createPayOrder((byte)2,payAmount,userDetails.getId());

            //生成微信支付请求参数
            Map<String, String> unifiedOrderParams = new HashMap<>();
            unifiedOrderParams.put("appid", "wx6071e813df70f188");
            unifiedOrderParams.put("mch_id", "1640989942");
            unifiedOrderParams.put("nonce_str", RandomStringUtils.randomAlphanumeric(32));
            unifiedOrderParams.put("body", "小说精品屋-plus");
            unifiedOrderParams.put("out_trade_no", outTradeNo.toString());
            unifiedOrderParams.put("total_fee", payAmount.toString());
            unifiedOrderParams.put("spbill_create_ip","");
            unifiedOrderParams.put("notify_url", "");
            unifiedOrderParams.put("trade_type", "NATIVE");

            //生成签名
            String sign = WechatPayUtil.generateSignature(unifiedOrderParams, wechatPayConfig.getKey());
            unifiedOrderParams.put("sign", sign);

            //发送微信支付请求
            String xml = WechatPayUtil.mapToXml(unifiedOrderParams);
            ...
            if ("SUCCESS".equals(returnCode)) {
                ...
                if ("SUCCESS".equals(resultCode)) {
                    ...
                    //生成二维码图片并返回给客户端
                    BufferedImage qrCodeImage = QRCodeUtil.createImage(codeUrl, null, false);
                    httpResponse.setContentType("image/png");
                    ImageIO.write(qrCodeImage, "png", httpResponse.getOutputStream());
                    httpResponse.getOutputStream().flush();
                    httpResponse.getOutputStream().close();
                } else {
                    //微信支付请求失败
                    throw new RuntimeException("微信支付请求失败：" + errCodeDes);
                }
            } else {
                //微信支付请求失败
                throw new RuntimeException("微信支付请求失败：" + returnMsg);
            }
        }
    }
